...
namespace visionary
{
	...
	bool PointCloudPlyWriter::WriteFormatPLY(const char* filename, const std::vector<PointXYZ>& points, bool useBinary, InvalidPointPresentation presentation)
	{
		return WriteFormatPLY(filename, points, std::vector<uint32_t>(), std::vector<uint16_t>(), useBinary, presentation);
	}

	bool PointCloudPlyWriter::WriteFormatPLY(const char* filename, const std::vector<PointXYZ>& points, const std::vector<uint32_t>& rgbaMap, bool useBinary, InvalidPointPresentation presentation)
	{
		return WriteFormatPLY(filename, points, rgbaMap, std::vector<uint16_t>(), useBinary, presentation);
	}

	bool PointCloudPlyWriter::WriteFormatPLY(const char* filename, const std::vector<PointXYZ>& points, const std::vector<uint16_t>& intensityMap, bool useBinary, InvalidPointPresentation presentation)
	{
		return WriteFormatPLY(filename, points, std::vector<uint32_t>(), intensityMap, useBinary, presentation);
	}

	bool PointCloudPlyWriter::WriteFormatPLY(const char* filename, const std::vector<PointXYZ>& points, const std::vector<uint32_t>& rgbaMap, const std::vector<uint16_t>& intensityMap, bool useBinary, InvalidPointPresentation presentation)
	{
		bool success = true;
		...
		std::stringstream strstream; // temporary data buffer stream 
		...
		if (useBinary == false)
		{
			...
			for (size_t i = 0; i < points.size(); i++)
			{
				...
				{
					if (std::isnan(point.x)) strstream << "0.0"; else strstream << point.x;
					strstream << " ";

					if (std::isnan(point.y)) strstream << "0.0"; else strstream << point.y;
					strstream << " ";

					if (std::isnan(point.z)) strstream << "0.0"; else strstream << point.z;
				}
				if (presentation == INVALID_AS_NAN || presentation == INVALID_AS_ZERO)
				{
					if (hasColors)
					{
						...
						strstream << " " << static_cast<uint32_t>(rgba[0]) << " " << static_cast<uint32_t>(rgba[1]) << " " << static_cast<uint32_t>(rgba[2]);
					}
					if (hasIntensities)
					{
		...
		else
		{
			...
			for (size_t i = 0; i < points.size(); i++)
			{
				...
				if (presentation == INVALID_AS_ZERO)
				{
					if (std::isnan(point.x)) point.x = 0.0f;
					if (std::isnan(point.y)) point.y = 0.0f;
					if (std::isnan(point.z)) point.z = 0.0f;
				}
				if (presentation == INVALID_AS_NAN || presentation == INVALID_AS_ZERO)
				{
