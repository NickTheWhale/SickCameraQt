...
namespace visionary
{
	...
	AuthenticationSecure::AuthenticationSecure(VisionaryControl& vctrl)
	...
	PasswordHash AuthenticationSecure::CreatePasswordHash(UserLevel userLevel, const std::string& password, const ChallengeRequest& challengeRequest, ProtocolType protocolType)
	{
		...
		sha256_process(&hashState, reinterpret_cast<const uint8_t*>(passwordWithPrefix.c_str()), static_cast<ulong32>(passwordWithPrefix.size()));
		if (protocolType == SUL2)
		{
			sha256_process(&hashState, reinterpret_cast<const uint8_t*>(separator.c_str()), static_cast<ulong32>(separator.size()));
	...
	ChallengeResponse AuthenticationSecure::CreateChallengeResponse(UserLevel userLevel, const std::string& password, const ChallengeRequest& challengeRequest, ProtocolType protocolType)
	{
		...
		sha256_process(&hashState, challengeRequest.challenge.data(), static_cast<ulong32>(challengeRequest.challenge.size()));
		sha256_done(&hashState, challengeResponse.data());

		return challengeResponse;
	}

	bool AuthenticationSecure::loginImpl(UserLevel userLevel, const std::string& password, const CoLaParameterReader& getChallengeResponse, ProtocolType protocolType)
	{
	...
	bool AuthenticationSecure::login(UserLevel userLevel, const std::string& password)
	{
		...
		CoLaParameterWriter getChallengeCommandBuilder = CoLaParameterWriter(CoLaCommandType::METHOD_INVOCATION, "GetChallenge");
		...
		{
			CoLaCommand getChallengeCommand = getChallengeCommandBuilder.parameterUSInt(static_cast<uint8_t>(userLevel)).build();
