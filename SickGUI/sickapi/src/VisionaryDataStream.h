...
namespace visionary
{
	...
	public:
		...
		/// \param[in] timeoutMs controls the socket timeout, default 5000ms
		///
		...
		bool open(const std::string& hostname, std::uint16_t port, std::uint64_t timeoutMs = 5000);
	...
	private:
		...
		bool parseSegmentBinaryData(const std::vector<uint8_t>::iterator itBuf, size_t bufferSize);
	};
