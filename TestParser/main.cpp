...
{
	
};
...
int main()
{
	...
	uint32_t a = buffer[pos++] | buffer[pos++] << 8 | buffer[pos++] << 16 | buffer[pos++] << 24;
	pos = 0;
	uint32_t b = buffer[pos++] << 24 | buffer[pos++] << 16 | buffer[pos++] << 8 | buffer[pos++];
...
uint32_t read_uint32_t(std::vector<uint8_t> buffer, size_t& pos)
{
	return buffer[pos++] << 24| buffer[pos++] << 16 | buffer[pos++] << 8 | buffer[pos++];
}

uint16_t read_uint16_t(std::vector<uint8_t> buffer, size_t& pos)
{
	return buffer[pos++] << 8 | buffer[pos++];
}

uint8_t read_uint8_t(std::vector<uint8_t> buffer, size_t& pos)
{
	return buffer[pos++];
}
...
void parseBuffer(std::vector<uint8_t> buffer)
{
	...
	while (true)
	{
		...
		// get telegram request id (for some reason its little endian??)
	...
	while (true)
	{
		...
		ver.buildNumber = read_uint32_t(buffer, pos);
		...
		uint16_t endpointSize = buffer[pos++] << 8 | buffer[pos++];
		for (size_t i = 0; i < endpointSize; ++i)
		{
			Endpoint endpoint;
			endpoint.port = "";
			endpoint.protocol = static_cast<Protocol>(buffer[pos++]);
			uint16_t arrSize = buffer[pos++] << 8 | buffer[pos++];
			for (size_t n = 0; n < arrSize; ++n)
			{
				...
				{
					pos += 2;
					endpoint.port = std::to_string(buffer[pos++] << 8 | buffer[pos++]);
				}
				else
				{
					readFlexString(buffer, pos);
