...
if args.source == "sensor":
    ...
    #convert and save 3 frames unless stopped before
    try:
        for i in range(3):
            streaming_device.getFrame()
            wholeFrame = streaming_device.frame
            myData.read(wholeFrame, args.output_millimeters)
            ...
            if myData.hasDepthMap:
                ...
                wCoordinates,distData = convertToPointCloud(myData.depthmap.distance,
                                                            myData.depthmap.intensity,
                                                            myData.depthmap.confidence,
                                                            myData.cameraParams, myData.xmlParser.stereo)
                
                writePointCloudToFile(os.path.join(directory, "world_coordinates{}.ply".format(i)), wCoordinates)

                # save as PNG
                saveDepthToPng( directory,distData, myData.depthmap.intensity,
                                myData.depthmap.confidence,
                                myData.cameraParams, i,
                                myData.xmlParser.stereo)
    ...
    for i in range(len(image_distance)):
        ...
        wCoordinates,_ = convertToPointCloud(image_distance[i], image_intensity[i], image_confidence[i], CamParams, isStereo)
        writePointCloudToFile(os.path.join(directory, "world_coordinates{}.ply".format(i)), wCoordinates)
