...
parser = argparse.ArgumentParser(description="Example for loading SSR data read recorded using a SICK Visionary-S device.")
...
for i in range(len(images_distance)):
    ...
    color_img = np.frombuffer(images_intensity[i], np.uint8).reshape(512,640,4)
